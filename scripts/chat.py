...
def main() -> None:
    ...
    if torch.cuda.is_available():
        ...
        print("No GPU detected — running on CPU in fp32 (will be slow, "
              "but a 1.5B model is usable — expect maybe 10-30s per reply).")
    # No device_map here: that's for models too large for one GPU. This 1.5B
    # model fits comfortably on any modern GPU — device_map="auto" was
    # sometimes offloading layers to CPU/disk as meta tensors, which silently
    # makes the LoRA adapter load into placeholder tensors and never actually
    # apply (a no-op copy), so the "tuned" model was actually just the base
    # model. Loading directly onto one device avoids that entirely.
